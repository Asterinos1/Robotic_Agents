...
public class Rat1 extends Robot {
  ...
  public void run() {
    ...
    //if we want to guide e-puck using the right wall we need 
    //to keep track of a few things.
    boolean isCloseToRightWall = false; //1) if we are close to the right wall
    boolean frontSideBlocked = false; //2) avoid frontal collision
    boolean rightCorner = false; //3) don't miss turns as we move along the wall
    ...
    while (step(timeStep) != -1) {
      ...
      //also distance[4] will be used for when we there might be an available
      //turn as we traverse across the wall.
      //distance[5] is right side sensor
      frontSideBlocked = distance[4] > 390 || distance[3] > 390;
      isCloseToRightWall = distance[5] > 300;
      rightCorner = distance[4] > 190;
      leftSpeed = maxSpeed;
      rightSpeed = maxSpeed;
      //setting the movement logic. we must stick to the right side 
      //at all times.
      if(frontSideBlocked){
        leftSpeed = maxSpeed;
        rightSpeed = -maxSpeed;
      }else{ 
        //when close to the right wall, we keep moving forward.
        if(isCloseToRightWall){
          leftSpeed = maxSpeed;
          rightSpeed = maxSpeed;
        }else{
          //if we're not close enough, we do a soft right turn to get close.
          leftSpeed = maxSpeed/7;
        }
        //if there's an available right turn as we move, we turn to go there.
        if(rightCorner){
          leftSpeed = maxSpeed;
          rightSpeed = maxSpeed/2.7;
        }
      }
      //recharging battery
